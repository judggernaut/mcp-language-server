...
// Glob pattern cache to avoid re-parsing the same pattern on every file
// event. Patterns come from a small, fixed set of server registrations, so a
// cache hit rate near 100% is expected; the size cap below is a safety valve
// against an unexpected pattern leak, not a normal code path.
var (
	globCache        sync.Map // pattern string -> *glob.Glob
	globCacheSize    int
	globCacheMu      sync.Mutex
	maxGlobCacheSize = 100
)

// parseGlobCached parses an LSP glob pattern with caching. If the cache grows
// past maxGlobCacheSize, it is cleared entirely and a warning is logged: that
// would mean patterns are being generated dynamically rather than reused,
// which this cache isn't designed for.
func parseGlobCached(pattern string) (*glob.Glob, error) {
	...
	globCacheMu.Lock()
	globCacheSize++
	cleared := globCacheSize > maxGlobCacheSize
	if cleared {
		globCache = sync.Map{}
		globCacheSize = 1
	}
	...
	if cleared {
		watcherLogger.Warn("Glob pattern cache exceeded %d entries, clearing cache (possible pattern leak)", maxGlobCacheSize)
	}
	return g, nil
}
...
func (w *WorkspaceWatcher) matchesPattern(path string, pattern protocol.GlobPattern) bool {
	...
	if basePath == "" {
		// Check if the pattern matches the full path or just the basename
		if g.Match(path) {
			return true
		}
	...
	// For relative patterns. GetBasePath() now returns a clean filesystem path
	// (see protocol.AsPattern), so no URI prefix needs stripping here.
